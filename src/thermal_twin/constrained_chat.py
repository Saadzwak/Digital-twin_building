"""M10 chat: only sealed M4/M9 evidence cards, with named-model safeguards."""
...
Intent = Literal["model_status", "topology", "metrics", "residual_episode", "scenario", "out_of_scope", "unsupported"]
...
def _compact(text: str) -> str:
    return "".join(character for character in _normalized(text) if character.isalnum())


def _named_topology(query: str) -> str | None:
    compact = _compact(query)
    for topology in reference_model_bank():
        full = _compact(topology.name)
        short = topology.name.split("_")[1] if topology.name.startswith("STD_") else topology.name.split("_")[1]
        if full in compact or short.lower() in compact:
...
def _unsupported_card() -> AnswerCard:
    return AnswerCard(
        ...
        run_source=None,
        scope_note="Le chat répond seulement sur l’état, la structure retenue, les métriques, le résidu et les scénarios conditionnels disponibles.",
    )
...
def write_chat_context(project_root: Path | str) -> Path:
    """Materialize fixed cards pinned to the current sealed dashboard source."""
    ...
    caveat = (
        ...
        if initialization_sensitive(verdict) else " La stabilité rapportée concerne uniquement les départs échantillonnés."
    )
    responses = {
        "model_status": {
            "text": f"Voici l’état d’identification de la structure retenue {payload.topology_label}.",
            "estimates": [asdict(item) for item in payload.identity_metrics],
            "scope_note": payload.identity_limit,
        },
        "topology": {
            "text": f"La structure RC effectivement retenue par le classement BIC de validation est {payload.topology_label}.",
            "estimates": [asdict(item) for item in payload.identity_metrics],
            "scope_note": payload.identity_limit,
        },
        "metrics": {
            "text": f"Voici les métriques sourcées de la structure retenue {payload.topology_label}.",
            "estimates": [asdict(item) for item in payload.identity_metrics],
    ...
    context = {
        ...
        "responses": responses,
        "scope": "Only dashboard IntervalEstimate cards from sealed M4/M5/M9 artifacts are eligible.",
    }
...
def answer(query: str, project_root: Path | str) -> AnswerCard:
    """Serve a scope-checked card, never a metric for a different named topology."""
    ...
    if context.get("run_source") != payload.run_source or context.get("topology_label") != payload.topology_label:
        return _blocked_card(M4ValidationError("Chat context does not match the sealed dashboard run/topology."))
    named = _named_topology(query)
    if named is not None and named != payload.topology_label:
        return AnswerCard(
            kind="refusal",
            text=f"Je ne peux pas attribuer la carte de métriques de {payload.topology_label} à {named}.",
            run_source=None,
            scope_note="Le chat ne possède pas de carte sourcée par structure non retenue; consultez le tableau de dispersion M4.",
        )
    ...
    return AnswerCard(
        kind="answer", text=str(content["text"]), run_source=str(context["run_source"]), estimates=estimates,
