...
def test_validated_dashboard_and_chat_surface_only_sourced_uncertain_cards() -> None:
    ...
    assert payload.topology_label in metric_card.text
    ...
    other = "STD_4R3C_two_masses_plus_air_shunt" if payload.topology_label != "STD_4R3C_two_masses_plus_air_shunt" else "STD_7R5C_four_masses_plus_shunts"
    named = answer(f"Quel est le BIC de {other} ?", root)
    assert named.kind == "refusal"
    assert other in named.text
